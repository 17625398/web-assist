...
import { createOpenCLITools, isOpenCLIEnabled } from "./tools/opencli-tool"
import { createPageAgentTools, isPageAgentEnabled } from "./tools/pageagent-tool"
import { createWebAutomationTools, isWebAutomationEnabled } from "./tools/web-automation-tool"
...
export const runMcpNormalChatMode = async (
  ...
  try {
    ...
    // Add OpenCLI tools if enabled
    if (await isOpenCLIEnabled()) {
      tools.push(...createOpenCLITools())
    }

    // Add Page-Agent tools if enabled
    if (await isPageAgentEnabled()) {
      tools.push(...createPageAgentTools())
    }

    // Add Web Automation tools if enabled
    if (await isWebAutomationEnabled()) {
      tools.push(...createWebAutomationTools())
    }
